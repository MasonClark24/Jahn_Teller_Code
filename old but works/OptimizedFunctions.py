...
def psi_t_function(psi, psi_t_vector_parts, t, indices_of_good_eigenvalues):
    """
    Given a base psi(0), turn it into psi(t)
    """

    num_eigenvectors = len(indices_of_good_eigenvalues)

    PSI_T = psi.copy()
    for i in range(len(psi_t_vector_parts)):
        # change psi parts to be psi(t)
        En = psi_t_vector_parts[i].E
        PSI_T[i] *= np.exp(-1j * En * t)
    ...
    separate_psis = np.array_split(PSI_T, num_eigenvectors)

    # add them together
    PSI_T = separate_psis[0]
    for i in range(1, num_eigenvectors):
        PSI_T += separate_psis[i]

    # Only need one copy of the vector parts!
    reduced_vector_parts = psi_t_vector_parts[:len(psi_t_vector_parts) // num_eigenvectors]
    ...
    return PSI_T, reduced_vector_parts
